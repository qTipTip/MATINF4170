...
def BSplineEvaluation(x, p, knots, mu=None):
    """
...
def demo():
    ...
    plt.scatter(
        *zip(*[(k, 0) for k in t[1:j + p + 2]]),
        s=50,
        zorder=100,
        color='grey')

    # Plots the dashed lines, signifying the non-active parts
    # uses the BSpline-class from bsplines.py.
    B = BasisSpline(p)
    for i in range(1, 4):
        ...
        x_values = np.linspace(t[j + 1], t[j + i + 1], 20)
    ...
    plt.legend(labels=[
        '$B_{1, 3}(x)$', '$B_{2, 3}(x)$', '$B_{3, 3}(x)$', '$B_{4, 3}(x)$'
    ])
    plt.savefig('active_b_splines.pdf', bbox_inches='tight')
    plt.show()

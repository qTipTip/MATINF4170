...
\begin{document}
    ...
    and the source code is given in \Cref{lst:nev_ait}. We wish to interpolate
    ...
    increasing \(N\), as can be seen in \Cref{fig:uniform}. The random
    ...
    large \(N\), as seen in \Cref{fig:random}.
...
immediately from the definition given in \Cref{eq:def}. Assume inductively that
...
\end{equation}
if the knots have to be emphasized. Using \Cref{eq:rec} we compute directly:
\begin{align*}
